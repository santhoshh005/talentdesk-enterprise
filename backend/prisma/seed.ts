import { PrismaClient, UserRoleEnum, JobStatus, ApplicationStageType, OfferStatus, InterviewStatus } from '@prisma/client';
...
async function main() {
  ...
  const locSF = await prisma.location.create({
    data: { organizationId: org.id, city: 'San Francisco', state: 'CA', country: 'USA', isRemote: false },
  });

  const locRemote = await prisma.location.create({
    data: { organizationId: org.id, city: 'Remote', state: 'Global', country: 'Worldwide', isRemote: true },
  });

  const locNY = await prisma.location.create({
    data: { organizationId: org.id, city: 'New York', state: 'NY', country: 'USA', isRemote: false },
  });
  ...
  const job1 = await prisma.job.create({
    data: {
      ...
      description: 'We are looking for a Senior Product Designer to lead design systems and core platform experience.',
      minSalary: 140000,
      maxSalary: 180000,
      skills: {
        create: [
  ...
  const candidateData = [
    {
      ...
      summary: 'Experienced lead designer specialized in building enterprise component libraries and micro-interactions.',
      jobId: job1.id,
      stageName: 'Interview',
    },
    {
      ...
      summary: 'Distributed systems engineer with 11+ years leading high-throughput event-driven microservices.',
      jobId: job2.id,
  ...
  for (const cData of candidateData) {
    ...
    const matchedStage = targetJob.pipelineStages.find((s) => s.name === cData.stageName) || targetJob.pipelineStages[0];

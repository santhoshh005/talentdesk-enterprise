...
import { Search, Download, Plus, MoreHorizontal, User, Copy, FileText, Target, Trash2, Edit3 } from "lucide-react";
...
function CandidatesPage() {
  ...
  const openEditCandidateModal = (c: any) => {
    ...
    setEditRole(c.role || "");
    setEditLoc(c.loc || "");
    setEditSkills(Array.isArray(c.skills) ? c.skills.join(", ") : "");
  };
  ...
  const downloadCSV = () => {
    ...
    filteredCandidates.forEach((c: any) => {
      ...
      lines.push(`"${c.name || ''}","${c.role || ''}","${c.email || ''}","${c.phone || ''}","${c.loc || ''}","${c.score || ''}","${skillsStr}"`);
    });
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={expFilter} onValueChange={setExpFilter}>
          <SelectTrigger className="w-[120px]"><SelectValue placeholder="Experience" /></SelectTrigger>
          <SelectContent>
          ...
          <TableBody>
            {isLoading ? (
              ...
              filteredCandidates.map((c: any) => {
                ...
                return (
                  <TableRow key={c.id} className="cursor-pointer hover:bg-secondary/20" onClick={() => setSelectedCandidateId(c.id)}>
                    <TableCell>
                    ...
                    <TableCell className="text-sm font-medium text-foreground">{c.role}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{c.loc}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{c.exp || `${c.experienceYears || 0} yrs`}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-52">
                          ...
                          <DropdownMenuItem onClick={() => openEditCandidateModal(c)} className="gap-2 cursor-pointer">
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <form onSubmit={handleSaveEditCandidate} className="space-y-4 pt-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="outline" onClick={() => setEditingCandidate(null)}>Cancel</Button>
